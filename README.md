...
```bash
movie-recommender-app-v2$ sam build
```
...
```bash
movie-recommender-app-v2$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
movie-recommender-app-v2$ sam local start-api
movie-recommender-app-v2$ curl http://localhost:3000/search
```
...
```bash
movie-recommender-app-v2$ sam logs -n InferenceFunction --stack-name movie-recommender-app-v2 --tail
```
...
```bash
aws cloudformation delete-stack --stack-name movie-recommender-app-v2
```
